...
import {SETTINGS,NODE_DATA,type BuildingId} from '../data/config';
...
const heights:Record<string,number>={tree:225,pine:262,rock:80,berry:61,grass:32,branch:25,pebble:24,wood:34,fire:69,canopy:122,hut:154,cabin:150,hero:78,hare:37,boar:68,tower:310};
export function setupFrames(scene:Phaser.Scene){const t=scene.textures.get('atlas');for(const [name,b]of Object.entries(FRAMES))if(!t.has(name))t.add(name,0,...b);}
...
export class WorldRenderer {
 nodeSprites:Phaser.GameObjects.Image[]=[];animalSprites:Phaser.GameObjects.Image[]=[];animalShadows:Phaser.GameObjects.Ellipse[]=[];buildingSprites=new Map<number,Phaser.GameObjects.Image>();
 ...
 sprite(frame:string,x:number,y:number){const img=this.scene.add.image(x,y,'atlas',frame).setOrigin(.5,.89).setDepth(y);const h=heights[frame]??64;img.setScale(h/img.height);return img;}
...
const BUILDING_RADIUS:Record<BuildingId,number>={fire:25,canopy:42,hut:48};
