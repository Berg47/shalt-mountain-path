...
export class Enemy {
 ...
 update(dt:number,player:Point,blocked:(x:number,y:number)=>boolean,damage:(n:number)=>void){
  ...
  if(this.state==='windup'){
   ...
   if(this.timer<=0){
    if(d<c.range+18&&this.cooldown<=0){damage(c.damage);this.cooldown=this.kind==='chaborz'?2.2:1.55;}
    this.state='recover';this.timer=this.kind==='chaborz'?.78:.45;
   }
   return;
  }
  if(this.state==='recover'){if(this.timer<=0)this.state=d<c.aggro*1.2?'chase':'idle';return;}
  if(d<c.range&&this.cooldown<=0){this.state='windup';this.timer=this.kind==='chaborz'?1.08:this.kind==='shield'?.68:.45;return;}
  if(d>c.aggro*1.75&&distance(this,this.home)>520){this.state='idle';return;}
  const angle=Math.atan2(player.y-this.y,player.x-this.x);const speed=c.speed*(d<c.range+45?.58:1);
