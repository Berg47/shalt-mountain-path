...
export class Player {
 ...
 health=100;maxHealth=100;stamina=100;maxStamina=100;hunger=90;temperature=36.8;invulnerable=0;attackTimer=0;actionTimer=0;
 update(dt:number,input:{x:number;y:number;sprint:boolean},blocked:(x:number,y:number)=>boolean){
  ...
  this.stamina=Math.max(0,Math.min(this.maxStamina,this.stamina+(sprint?-23:13)*dt));
 }
