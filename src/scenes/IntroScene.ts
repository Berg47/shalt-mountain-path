...
export class IntroScene extends Phaser.Scene {
 ...
 create(){
  ...
  this.root.addEventListener('click',e=>{const target=(e.target as HTMLElement).closest<HTMLElement>('button');if(!target)return;this.audio.unlock();const a=target.dataset.action;if(a==='new'){if(SaveSystem.read())this.confirmNew();else this.next();}else if(a==='confirm-new'){this.index=-1;this.next();}else if(a==='continue')this.startGame(true);else if(a==='recover'){const restored=SaveSystem.restoreBackup();if(restored)this.startGame(true);else this.title();}else if(a==='next')this.next();else if(a==='skip')this.startGame(false);else if(a==='back')this.title();else if(a==='intro-help')this.help();},{signal:this.abort.signal});
