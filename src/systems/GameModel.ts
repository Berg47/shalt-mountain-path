import {SETTINGS,REGIONS,NODE_DATA,ITEMS,RECIPES,BUILDINGS,ANIMALS,ENEMIES,type RecipeId,type BuildingId,type ResourceId,type ToolId,type EnemyKind} from '../data/config';
...
export type GameEvent={type:'toast'|'gather'|'hit'|'attack'|'damage'|'build'|'cook'|'level'|'discover'|'death'|'trial'|'adulthood';text?:string;x?:number;y?:number;kind?:string;amount?:number};
...
export class GameModel {
 ...
 interact(){
  ...
  const i=this.interaction();if(!i){this.toast('Подойди ближе к ресурсу, добыче или лагерю');return;}
  if(i.kind==='node'){
   ...
   if(!gained)this.toast('Не хватает места. Добыча останется здесь.');else this.player.actionTimer=.45;
