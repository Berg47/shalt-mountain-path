...
export class UI {
 ...
 renderPanel(){
  ...
  if(this.panel==='inventory'){
   ...
   body=`<div class="valley-map"><div class="map-river"></div><span class="map-north">СЕВЕР ↑</span>${REGIONS.map(r=>`<span class="map-place ${m.discovered.has(r.id)?'discovered':''}" style="left:${r.x/SETTINGS.world.width*100}%;top:${r.y/SETTINGS.world.height*100}%"><i></i>${m.discovered.has(r.id)?r.name:'Неизведано'}</span>`).join('')}<span class="map-elder" style="left:${ELDER_QUEST.tower.x/SETTINGS.world.width*100}%;top:${ELDER_QUEST.tower.y/SETTINGS.world.height*100}%" title="Старец у центральной башни"><i></i>Старец</span><span class="map-you" style="left:${m.player.x/SETTINGS.world.width*100}%;top:${m.player.y/SETTINGS.world.height*100}%"><i></i>Ты</span>${m.buildings.objects.map(b=>`<span class="map-camp" style="left:${b.x/SETTINGS.world.width*100}%;top:${b.y/SETTINGS.world.height*100}%" title="${BUILDINGS[b.kind].name}">${icon(b.kind)}</span>`).join('')}</div><p class="panel-note">Реку можно пересечь у Каменного брода. На востоке теперь открывается путь к башенному селению. Вход в пещеру открыт.${m.caveFirstDefeated?` Чёрный Волк снова появится с ${m.caveNextAvailableDay}-го игрового дня.`:''}</p>`;
 ...
 update(force=false){
  ...
  const marker=this.root.querySelector<HTMLElement>('#minimap-player')!;marker.style.left=`${p.x/SETTINGS.world.width*100}%`;marker.style.top=`${p.y/SETTINGS.world.height*100}%`;
